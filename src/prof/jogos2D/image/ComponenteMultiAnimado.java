 ...
 * que se tem várias direções, por exemplo, e se escolhe a animação
 * consoante a direção do elemento
 * 
...
public class ComponenteMultiAnimado extends ComponenteSimples {
	 ...
	 * @param delay      factor que serve para controlar a anima��o (quanto maior
	 *                   menor a velocidade da anima��o)
	 * @throws IOException
	 */
	public ComponenteMultiAnimado(String fichImagem, int nAnims, int nFrames, int delay) throws IOException {
		super(fichImagem);
	 ...
	 */
	public ComponenteMultiAnimado(BufferedImage img, int nAnims, int nFrames, int delay) {
		produzirFrames(nAnims, nFrames, img);
		this.delay = delay;
	}
	 ...
	 *                a velocidade da anima��o)
	 */
	public ComponenteMultiAnimado(Image img[][], int delay) {
		frames = img;
		// nAnims = img.length;
	...
	public void desenhar(Graphics g) {
		...
		// g.drawImage(frames[k][i], getPosicao().x+i*getComprimento(),
		// getPosicao().y+k*getAltura(), null);
	...
	public ComponenteMultiAnimado clone() {
		ComponenteMultiAnimado sp = new ComponenteMultiAnimado(frames, delay);
